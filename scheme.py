...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        op_ator = scheme_eval(first,env)
        if isinstance(op_ator, MacroProcedure):
            return scheme_eval(op_ator.apply_macro(rest, env), env)
        return scheme_apply(op_ator, rest.map(lambda expr: scheme_eval(expr,env)), env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def helper(formals, vals):
            if formals is nil:
                return new_frame
            elif formals.rest is nil and not vals.rest is nil:
                raise SchemeError
            elif vals.rest is nil and not formals.rest is nil:
                raise SchemeError
            elif formals.rest is nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        def helper_1(target, env):
            if target.rest is nil:
                return nil
            return Pair(target.rest.first, helper_1(target.rest, env))
        def helper(expressions, env):
            if expressions.rest is nil:
                return nil
            return Pair(expressions.rest.first, helper(expressions.rest, env))
        lambda_build = LambdaProcedure(helper_1(target,env), helper(expressions, env), env)
        env.define(target.first, lambda_build)
        return target.first
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    def helper(expressions,env):
        if expressions.rest is nil:
            return nil
        return Pair(expressions.rest.first, helper(expressions.rest, env))
    return LambdaProcedure(formals, helper(expressions, env), env)
    # END PROBLEM 8
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if expressions.rest is nil:
            return first
        elif not first and first is not 0:
            return first
        else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    def helper(bindings, env):
        if bindings is nil:
            return nil
        else:
            check_form(bindings.first, 2, 2)
            return Pair(bindings.first.first, helper(bindings.rest, env))
    def helper_2(bindings, env):
        if bindings is nil:
            return nil
        return Pair(scheme_eval(bindings.first.rest.first, env), helper_2(bindings.rest, env))
    formal_lst = helper(bindings,env)
    vals_lst = helper_2(bindings, env)
    check_formals(formal_lst)
    return env.make_child_frame(formal_lst, vals_lst)
    # END PROBLEM 14
    ...
    if scheme_symbolp(target):
        ...
        def helper(expressions, env):
            if expressions.rest is nil:
                return nil
            return Pair(expressions.rest.first, helper(expressions.rest, env))
        macro_build = MacroProcedure(helper_1(target,env), helper(expressions, env), env)
        env.define(target.first, macro_build)
        return target.first
        # END PROBLEM 9
    else:
        bad_target = target.first if isinstance(target, Pair) else target
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 15
    def body_builder(expressions, env):
        if expressions.rest is nil:
            return nil
        return Pair(expressions.rest.first, body_builder(expressions.rest, env))
    body = body_builder(expressions, env)
